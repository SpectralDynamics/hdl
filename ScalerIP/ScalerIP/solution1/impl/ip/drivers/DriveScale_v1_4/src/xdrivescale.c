...
void XDrivescale_Set_fClipValue(XDrivescale *InstancePtr, u32 Data) {
    ...
    XDrivescale_WriteReg(InstancePtr->Control_BaseAddress, XDRIVESCALE_CONTROL_ADDR_FCLIPVALUE_DATA, Data);
}
...
u32 XDrivescale_Get_fClipValue(XDrivescale *InstancePtr) {
    ...
    Data = XDrivescale_ReadReg(InstancePtr->Control_BaseAddress, XDRIVESCALE_CONTROL_ADDR_FCLIPVALUE_DATA);
    return Data;
}
...
u32 XDrivescale_Write_calibratedStep_Words(XDrivescale *InstancePtr, int offset, word_type *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(int *)(InstancePtr->Control_BaseAddress + XDRIVESCALE_CONTROL_ADDR_CALIBRATEDSTEP_BASE + (offset + i)*4) = *(data + i);
    }
    return length;
}
...
u32 XDrivescale_Read_calibratedStep_Words(XDrivescale *InstancePtr, int offset, word_type *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(data + i) = *(int *)(InstancePtr->Control_BaseAddress + XDRIVESCALE_CONTROL_ADDR_CALIBRATEDSTEP_BASE + (offset + i)*4);
    }
    return length;
}
...
u32 XDrivescale_Write_calibratedStep_Bytes(XDrivescale *InstancePtr, int offset, char *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(char *)(InstancePtr->Control_BaseAddress + XDRIVESCALE_CONTROL_ADDR_CALIBRATEDSTEP_BASE + offset + i) = *(data + i);
    }
    return length;
}
...
u32 XDrivescale_Read_calibratedStep_Bytes(XDrivescale *InstancePtr, int offset, char *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(data + i) = *(char *)(InstancePtr->Control_BaseAddress + XDRIVESCALE_CONTROL_ADDR_CALIBRATEDSTEP_BASE + offset + i);
    }
    return length;
}
...
u32 XDrivescale_Write_calibratedOffset_Words(XDrivescale *InstancePtr, int offset, word_type *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(int *)(InstancePtr->Control_BaseAddress + XDRIVESCALE_CONTROL_ADDR_CALIBRATEDOFFSET_BASE + (offset + i)*4) = *(data + i);
    }
    return length;
}
...
u32 XDrivescale_Read_calibratedOffset_Words(XDrivescale *InstancePtr, int offset, word_type *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(data + i) = *(int *)(InstancePtr->Control_BaseAddress + XDRIVESCALE_CONTROL_ADDR_CALIBRATEDOFFSET_BASE + (offset + i)*4);
    }
    return length;
}
...
u32 XDrivescale_Write_calibratedOffset_Bytes(XDrivescale *InstancePtr, int offset, char *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(char *)(InstancePtr->Control_BaseAddress + XDRIVESCALE_CONTROL_ADDR_CALIBRATEDOFFSET_BASE + offset + i) = *(data + i);
    }
    return length;
}
...
u32 XDrivescale_Read_calibratedOffset_Bytes(XDrivescale *InstancePtr, int offset, char *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(data + i) = *(char *)(InstancePtr->Control_BaseAddress + XDRIVESCALE_CONTROL_ADDR_CALIBRATEDOFFSET_BASE + offset + i);
    }
    return length;
}
...
u32 XDrivescale_Write_OffsetCounts_Words(XDrivescale *InstancePtr, int offset, word_type *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(int *)(InstancePtr->Control_BaseAddress + XDRIVESCALE_CONTROL_ADDR_OFFSETCOUNTS_BASE + (offset + i)*4) = *(data + i);
    }
    return length;
}
...
u32 XDrivescale_Read_OffsetCounts_Words(XDrivescale *InstancePtr, int offset, word_type *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(data + i) = *(int *)(InstancePtr->Control_BaseAddress + XDRIVESCALE_CONTROL_ADDR_OFFSETCOUNTS_BASE + (offset + i)*4);
    }
    return length;
}
...
u32 XDrivescale_Write_OffsetCounts_Bytes(XDrivescale *InstancePtr, int offset, char *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(char *)(InstancePtr->Control_BaseAddress + XDRIVESCALE_CONTROL_ADDR_OFFSETCOUNTS_BASE + offset + i) = *(data + i);
    }
    return length;
}
...
u32 XDrivescale_Read_OffsetCounts_Bytes(XDrivescale *InstancePtr, int offset, char *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(data + i) = *(char *)(InstancePtr->Control_BaseAddress + XDRIVESCALE_CONTROL_ADDR_OFFSETCOUNTS_BASE + offset + i);
    }
    return length;
}
