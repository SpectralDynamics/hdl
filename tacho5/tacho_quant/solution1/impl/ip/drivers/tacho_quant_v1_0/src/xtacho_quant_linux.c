...
typedef struct {
    ...
    XTacho_quant_uio_map maps[ MAX_UIO_MAPS ];
} XTacho_quant_uio_info;

/***************** Variable Definitions **************************************/
static XTacho_quant_uio_info uio_info;
...
static int uio_info_read_name(XTacho_quant_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XTacho_quant_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XTacho_quant_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XTacho_quant_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XTacho_quant_Initialize(XTacho_quant *InstancePtr, const char* InstanceName) {
	XTacho_quant_uio_info *InfoPtr = &uio_info;
